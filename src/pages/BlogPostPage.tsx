    ...
    return (
      <PageLayout title="Loading..." description="Loading blog post.">
        <div className="text-center py-8">
          <p className="text-lavender">Loading blog post...</p>
        </div>
      </PageLayout>
    );
  }
  ...
  return (
    ...
    >
      <div className="w-full">
        <div className="bg-navy/40 rounded-lg p-4 md:p-8">
          <div className="max-w-4xl mx-auto">
            <Link to="/blog" className="inline-flex items-center text-coral hover:text-mint mb-6">
              <ArrowLeft className="mr-2 h-5 w-5" />
              Back to Blog
            </Link>
            
            <h1 className="text-3xl md:text-4xl font-bold text-mint mb-4">{post.title}</h1>
            
            {post.metaDescription && (
              <p className="text-lavender/80 border-l-4 border-coral pl-4 italic my-4">
                {post.metaDescription}
              </p>
            )}
            
            <div className="flex flex-wrap items-center gap-4 text-peach mb-4">
            ...
            {post.imageUrl && (
              <img 
                src={post.imageUrl} 
                alt={post.title} 
                className="rounded-lg mb-8 w-full h-auto object-cover max-h-96"
              />
            )}

            <div className="flex flex-wrap items-center gap-3 mb-8 p-4 bg-navy/30 rounded-lg">
              <span className="font-semibold flex items-center text-mint">
              ...
              <div className="flex gap-2">
                ...
                >
                  ...
                  {copySuccess && (
                    <span className="absolute -top-8 left-1/2 -translate-x-1/2 bg-mint text-navy text-xs rounded px-2 py-1 whitespace-nowrap">
                      {copySuccess}
                    </span>
                  )}
                </button>
            ...
            <article 
              className="prose prose-invert prose-lg max-w-none 
                prose-headings:text-mint prose-h1:text-3xl prose-h2:text-2xl prose-h3:text-xl
                prose-p:text-lavender/90 prose-p:leading-relaxed
                prose-a:text-coral hover:prose-a:text-mint prose-a:transition-colors
                prose-strong:text-peach prose-em:text-peach/80
                prose-ul:text-lavender/80 prose-ol:text-lavender/80
                prose-blockquote:border-coral prose-blockquote:text-peach/80
                space-y-6"
